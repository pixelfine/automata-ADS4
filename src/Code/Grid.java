...
public class Grid
{
    ...
    public static Grid create(Object[][] grid, int posX, int posY, int dir) {
        int sizeX = grid.length;
        int sizeY = sizeX == 0 ? 0 : grid[0].length;
        Object[][] gridCopy = new Object[sizeX][sizeY];
    ...
    public static Grid parseGrid(String exeName, Reader reader) {
        ...
        try {
            ...
            Object[][] grid = new Object[sizeX][sizeY];
            for (int j = sizeY - 1; j >= 0; j --) {
                for (int i = 0; i < sizeX; i ++) {
                	if(scanner.hasNext("#")) {
                		grid[i][j] = scanner.next();
                	}
                	else grid[i][j] = scanner.nextInt();
                }
            }
    ...
    public void move(int moveX, int moveY, int rot) {
    	SaveActions save = new SaveActions(this);
	    setPosX(posX + moveX);
	    setPosY(posY + moveY);
	    dir = dir.rotate(rot);
	    if(grid[posX][posY].equals("#")) {
	        setPosX(save.posX);
	        setPosY(save.posY);
	    }
    }
    ...
    public int lire() {
        return (Integer)getValue(getPosX(), getPosY());
    }
    ...
    public String toString() {
        ...
        for (int y = sizeY - 1; y >= 0; y --) {
            for (int x = 0; x < sizeX; x ++) {
                if(!(grid[x][y] instanceof String)) {
                	builder.append(String.format("%" + lengths[x] + "d", grid[x][y]));
                }
                else {
